...
$tense=($count==1) ? "" : "votes"; //plural form votes/vote
...
if ($static == 'static') {
		...
		$static_rater[] .= '<ul id="unit_ul'.$id.' style="width:'.$rating_unitwidth*$units.'px;">';
		$static_rater[] .= '<li class="current-rating" style="width:'.$rating_width.'px;">Currently '.$rating2.'/'.$units.'</li>';
		$static_rater[] .= '</ul>';
		$static_rater[] .= '<p>'. 'Calificaci&oacute;n: '.$rating1.''.$units.'</p>';
		$static_rater[] .= '</div>';
		$static_rater[] .= '</div>'."\n\n";
      ...
      $rater.='  <p';
      if($voted){ $rater.=' '; }
      $rater.='>'.' Calificaci&oacute;n:  '.$rating1.'/'.$units.'';
      $rater.='  </p>';
      $rater.='</div>';
      $rater.='</div>';
      return $rater;
